...
must_have_skills_stream AS (SELECT * FROM {{ source('dbt_agent', 'stg_job_ads_daily_must_have_skills') }}),
must_have_work_exp_stream AS (SELECT * FROM {{ source('dbt_agent', 'stg_job_ads_daily_must_have_work_exp') }}),
must_have_edu_level_stream AS (SELECT * FROM {{ source('dbt_agent', 'stg_job_ads_daily_must_have_education_level') }})
--Remove all removed
SELECT
    id,
    number_of_vacancies,
